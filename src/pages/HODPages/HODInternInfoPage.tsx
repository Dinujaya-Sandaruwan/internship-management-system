...
import { FaSearch, FaFilter, FaEye, FaFileAlt, FaClipboardCheck, FaDownload, FaEdit, FaSort, FaSortUp, FaSortDown, FaUsers, FaGraduationCap, FaBuilding, FaClock, FaCheckCircle, FaExclamationTriangle, FaTimes } from 'react-icons/fa';
...
type SortField = 'name' | 'department' | 'status' | 'progress' | 'gpa' | 'startDate' | 'evaluationStatus';
...
const HODInternInfoPage: React.FC = () => {
  ...
  const filterOptions = useMemo(() => {
    ...
    const evaluationStatuses = [...new Set(interns.map(intern => intern.evaluationStatus))];
    const workModes = [...new Set(interns.map(intern => intern.workMode))];
    const compensationTypes = [...new Set(interns.map(intern => intern.compensationType))];
  ...
  useEffect(() => {
    const filtered = interns.filter(intern => {
      ...
      const matchesDepartment = filters.department === '' || intern.department === filters.department;
      const matchesStatus = filters.status === '' || intern.status === filters.status;
      const matchesEvaluationStatus = filters.evaluationStatus === '' || intern.evaluationStatus === filters.evaluationStatus;
      const matchesWorkMode = filters.workMode === '' || intern.workMode === filters.workMode;
      const matchesCompensationType = filters.compensationType === '' || intern.compensationType === filters.compensationType;
      const matchesUniversity = filters.university === '' || intern.university === filters.university;
      const matchesYear = filters.year === '' || intern.year === filters.year;
      const matchesSkills = filters.skills === '' || intern.skills.some(skill => 
        skill.toLowerCase().includes(filters.skills.toLowerCase())
      );

      return matchesSearch && matchesDepartment && matchesStatus && matchesEvaluationStatus &&
             matchesWorkMode && matchesCompensationType && matchesUniversity && matchesYear && matchesSkills;
    });
  ...
  const paginatedInterns = filteredInterns.slice(startIndex, startIndex + itemsPerPage);
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'completed': return <FaGraduationCap className="status-icon completed" />;
      case 'pending': return <FaClock className="status-icon pending" />;
      default: return <FaExclamationTriangle className="status-icon warning" />;
    }
  };

  const getEvaluationStatusIcon = (status: string) => {
    switch (status) {
      case 'completed': return <FaCheckCircle className="eval-icon completed" />;
      case 'pending': return <FaClock className="eval-icon pending" />;
      default: return <FaExclamationTriangle className="eval-icon warning" />;
    }
  };
  ...
  const renderInternCard = (intern: Intern) => (
    <div key={intern.id} className="intern-card">
      <div className="intern-card-header">
        ...
        />
        <img src={intern.profileImage} alt={intern.name} className="intern-avatar" />
        ...
        <div className="intern-status">
          ...
          <span className={`status-text ${intern.status}`}>{intern.status}</span>
        </div>
      </div>
      ...
      <div className="intern-card-body">
        ...
        <div className="intern-evaluation">
          ...
          <div className="reports-info">
            ...
            <span>{intern.reportsSubmitted}/{intern.totalReports} Reports</span>
          </div>
        </div>
      </div>
        ...
        <thead>
          <tr>
            <th>
              <input
                type="checkbox"
                checked={selectedInterns.size === paginatedInterns.length && paginatedInterns.length > 0}
            ...
            <th onClick={() => handleSort('evaluationStatus')} className="sortable">
            ...
            <tr key={intern.id}>
              ...
              <td>
                <div className="table-name-cell">
                  <img src={intern.profileImage} alt={intern.name} className="table-avatar" />
    ...
    <div className="hod-intern-info-page">
      ...
      <div className="intern-info-content">
        ...
        <div className="page-stats">
          ...
          <div className="stat-card">
            ...
            <div className="stat-info">
              <span className="stat-number">{filteredInterns.filter(i => i.status === 'active').length}</span>
          ...
          <div className="stat-card">
            ...
            <div className="stat-info">
              <span className="stat-number">{filteredInterns.filter(i => i.status === 'completed').length}</span>
          ...
          <div className="view-controls">
            ...
            {selectedInterns.size > 0 && (
              <div className="bulk-actions">
                <span className="selected-count">{selectedInterns.size} selected</span>
      ...
      {showDetailModal && selectedIntern && (
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="intern-detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-content">
              <div className="intern-profile-section">
                <img src={selectedIntern.profileImage} alt={selectedIntern.name} className="profile-image" />
              ...
              <div className="details-grid">
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">{selectedIntern.internshipTitle}</span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Duration:</span>
                    <span className="value">{selectedIntern.startDate} to {selectedIntern.endDate}</span>
                  </div>
                  <div className="detail-row">
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">{selectedIntern.reportsSubmitted}/{selectedIntern.totalReports}</span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Evaluation Status:</span>
                    <span className="value">{selectedIntern.evaluationStatus}</span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Mid Evaluation:</span>
                    <span className="value">{selectedIntern.midEvaluation ? 'Completed' : 'Pending'}</span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Final Evaluation:</span>
                    <span className="value">{selectedIntern.endEvaluation ? 'Completed' : 'Pending'}</span>
                  </div>
                </div>
